...
typeNode::typeNode(typenode_types type, typeNode *next, int val)
{
    this->type = type;
    this->next = next;
    this->val = val;
...
void SymbolTableEntry::printInitialValue()
{
    if (type->type == FUNCTION)
    {
        cout << setw(20) << "not applicable"
             << " ";
    }
    ...
    {
        cout << setw(20) << (*((int *)initial_value)) << " ";
    }
    else if (type->type == CHAR1)
    {
        cout << setw(20) << (*((char *)initial_value)) << " ";
    }
    else if (type->type == PTR)
    {
        cout << setw(20) << string((char *)(initial_value)) << " ";
    }
}
...
void SymbolTableEntry::printEntry()
{
    ...
    if (!nested_table)
        ...
        cout << setw(20) << (nested_table->STName);
}
...
void convBool2Int(Expression *e)
{
    if (e->type == "bool")
    {
        ...
        q = new quadTableEntry("", "", to_string(quad_table->quadArray.size() + 1), GOTO, 0, 0, 0);
        quad_table->emit(q);

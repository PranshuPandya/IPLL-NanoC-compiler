...
{
       0,   119,   119,   124,   132,   150,   161,   171,   198,   205,
     218,   240,   248,   255,   260,   266,   272,   281,   285,   371,
     375,   379,   383,   387,   394,   415,   445,   474,   506,   510,
     526,   545,   549,   568,   588,   608,   631,   635,   657,   682,
     686,   701,   705,   720,   724,   754,   758,   782,   789,   793,
     797,   809,   812,   815,   821,   835,   841,   848,   858,   877,
     899,   908,   912,   915,   927,   940,   949,   955,   963,   970,
     974,   979,   983,   987,   994,  1001,  1005,  1013,  1018,  1024,
    1028,  1035,  1040,  1047,  1057,  1071,  1081,  1095,  1102,  1111,
    1112,  1116,  1117,  1121,  1129,  1142
};
#endif
...
#line 151 "A6_36.y"
    {
        ...
        quadTableEntry* q = new quadTableEntry(to_string((yyvsp[0].intval)),"",tmp->name,ASSIGN,nullptr, nullptr, tmp);
        quad_table->emit(q);
    }
#line 1332 "A6_36.tab.c"
    break;
...
#line 162 "A6_36.y"
    {
        ...
        tmp->initial_value = g;    
        (yyval.exp)->loc = tmp;
        quadTableEntry* q = new quadTableEntry(to_string((yyvsp[0].char_value)),"",tmp->name,ASSIGN_CHAR,nullptr, nullptr, tmp);
        quad_table->emit(q);    
    }
#line 1346 "A6_36.tab.c"
    break;
...
#line 172 "A6_36.y"
    {
        ...
        (yyval.exp)->loc = tmp;
        quad_table->emit(new quadTableEntry(to_string(stringLiterals.size()),"",(yyval.exp)->loc->name,STRING_ASSIGN,0,0,(yyval.exp)->loc));

        stringLiterals.push_back((yyvsp[0].str_value));
        // quadTableEntry *q = new quadTableEntry(to_string(stringLiterals.size()),"",$$->loc->name,STRING_ASSIGN,0,0,$$->loc);
        // // char* g = new char($1);
        // // tmp->initial_value = g;
    }
#line 1377 "A6_36.tab.c"
    break;
...
#line 206 "A6_36.y"
    {
        ...
        (yyval.arr)->btype = (yyvsp[0].exp)->type;
        (yyval.arr)->truelist = (yyvsp[0].exp)->truelist;
        (yyval.arr)->falselist = (yyvsp[0].exp)->falselist;
        (yyval.arr)->nextlist = (yyvsp[0].exp)->nextlist;
    }
#line 1402 "A6_36.tab.c"
    break;
        ...
        switch((yyvsp[-1].uop))
        {
            case U_ADDR:
            {
                ...
                if((yyvsp[0].arr)->atype == "arr"){
                    quadTableEntry *q = new quadTableEntry((yyvsp[0].arr)->array->name,(yyvsp[0].arr)->loc->name,(yyval.arr)->array->name,U_ADDR,(yyvsp[0].arr)->array,(yyvsp[0].arr)->loc,(yyval.arr)->array);
                    quad_table->emit(q);
                }else{
                ...
                // if($2->atype == "arr"){
                //     SymbolTableEntry* te = cur_symbol_table->gentemp(PTR); 
                //     te->type->next = copyType($2->loc->type);
                //     quadTableEntry *q = new quadTableEntry($$->array->name,$2->loc->name,te->name,PLUS,$$->array,$2->loc,te);
                //     quad_table->emit(q);                    
                //     $$->array = te;
                // }
                break;
            ...
            {
                if ((yyvsp[0].arr)->atype == "arr"){
                    (yyval.arr)->array = cur_symbol_table->gentemp((yyvsp[0].arr)->array->type->next->next->type);
                    (yyval.arr)->loc = (yyval.arr)->array;

                    quadTableEntry *q = new quadTableEntry((yyvsp[0].arr)->array->name,(yyvsp[0].arr)->loc->name,(yyval.arr)->loc->name,U_STAR,(yyvsp[0].arr)->array,(yyvsp[0].arr)->loc,(yyval.arr)->loc);
                    quad_table->emit(q);
                }else{
                    (yyval.arr)->atype = "ptr";
            ...
            {
                if((yyvsp[0].arr)->atype == "arr"){
                    SymbolTableEntry* te = cur_symbol_table->gentemp((yyvsp[0].arr)->loc->type->type); 
                    te->type->next = (yyvsp[0].arr)->array->type->next->next;

                    quadTableEntry *q = new quadTableEntry((yyvsp[0].arr)->array->name,(yyvsp[0].arr)->loc->name,te->name,EQ_BRT,(yyvsp[0].arr)->array,(yyvsp[0].arr)->loc,te);
                    quad_table->emit(q);                    
                    
                    (yyval.arr)->array = cur_symbol_table->gentemp(te->type->type);
                    q = new quadTableEntry(te->name,"",(yyval.arr)->array->name,U_MINUS,te,nullptr,(yyval.arr)->array);
                    quad_table->emit(q);
                }else{
...
#line 395 "A6_36.y"
    {
        ...
        (yyval.exp)->type = (yyvsp[0].arr)->btype;
        (yyval.exp)->truelist = (yyvsp[0].arr)->truelist;
        (yyval.exp)->falselist = (yyvsp[0].arr)->falselist;
        (yyval.exp)->nextlist = (yyvsp[0].arr)->nextlist;
    }
#line 1644 "A6_36.tab.c"
    break;
...
#line 798 "A6_36.y"
    {
        ...
        if((yyvsp[-2].ste)->category == SymbolTableEntry::GLOBAL){
            global_initialized.insert((yyvsp[-2].ste)->name);
        }
        quadTableEntry *q = new quadTableEntry((yyvsp[0].ste)->name,"",(yyvsp[-2].ste)->name,ASSIGN,(yyvsp[0].ste),nullptr,(yyvsp[-2].ste));
        quad_table->emit(q);
    }
#line 2110 "A6_36.tab.c"
    break;
...
#line 822 "A6_36.y"
    {
        ...
        // t->next = $2->type;
        if((yyvsp[0].ste)->type->type == FUNCTION || (yyvsp[0].ste)->type->type == ARRAY){
            t->next = (yyvsp[0].ste)->type->next;
            (yyvsp[0].ste)->type->next = t;
            (yyvsp[0].ste)->size = compute_type_size((yyvsp[0].ste)->type);
        }else{
...
#line 859 "A6_36.y"
    {
        ...
        (yyvsp[-4].ida).loc->category = SymbolTableEntry::FUNCT;
...
#line 878 "A6_36.y"
    {
        ...
        (yyvsp[-3].ida).loc->category = SymbolTableEntry::FUNCT;
...
#line 928 "A6_36.y"
    {
        (yyvsp[0].ida).loc = cur_symbol_table->lookup(((yyvsp[0].ida).var));
        (yyvsp[0].ida).loc->update(new typeNode(cur_type), (yyvsp[0].ida).loc->initial_value, (yyvsp[0].ida).loc->nested_table);     
        (yyvsp[0].ida).loc->category = SymbolTableEntry::PARAMETER;
        typeNode* t = new typeNode(PTR);
...
#line 1122 "A6_36.y"
    {
        cur_symbol_table = global_symbol_table;
        quad_table->emit(new quadTableEntry((yyvsp[-2].ste)->name,"","",FUCNEND,(yyvsp[-2].ste),0,0));
    }
#line 2547 "A6_36.tab.c"
    break;
...
#line 2571 "A6_36.tab.c"
    break;


#line 2575 "A6_36.tab.c"

...
primary_expression:
    ...
    |CHARACTER_CONST 
    {
        ...
        tmp->initial_value = g;    
        $$->loc = tmp;
        quadTableEntry* q = new quadTableEntry(to_string($1),"",tmp->name,ASSIGN_CHAR,nullptr, nullptr, tmp);
        quad_table->emit(q);    
    }
    |STRING_LITERAL 
    {
        ...
        tmp->initial_value = $1;
        $$->loc = tmp;
        quad_table->emit(new quadTableEntry(to_string(stringLiterals.size()),"",$$->loc->name,STRING_ASSIGN,0,0,$$->loc));
    ...
    |unary_operator unary_expression
    {
        ...
        switch($1)
        {
            case U_ADDR:
            {
                ...
                if($2->atype == "arr"){
                    quadTableEntry *q = new quadTableEntry($2->array->name,$2->loc->name,$$->array->name,U_ADDR,$2->array,$2->loc,$$->array);
                    quad_table->emit(q);
                }else{
            ...
            {
                if ($2->atype == "arr"){
                    $$->array = cur_symbol_table->gentemp($2->array->type->next->next->type);
                    $$->loc = $$->array;

                    quadTableEntry *q = new quadTableEntry($2->array->name,$2->loc->name,$$->loc->name,U_STAR,$2->array,$2->loc,$$->loc);
                    quad_table->emit(q);
                }else{
                    $$->atype = "ptr";
            ...
            {
                if($2->atype == "arr"){
                    SymbolTableEntry* te = cur_symbol_table->gentemp($2->loc->type->type); 
                    te->type->next = $2->array->type->next->next;

                    quadTableEntry *q = new quadTableEntry($2->array->name,$2->loc->name,te->name,EQ_BRT,$2->array,$2->loc,te);
                    quad_table->emit(q);                    
                    
                    $$->array = cur_symbol_table->gentemp(te->type->type);
                    q = new quadTableEntry(te->name,"",$$->array->name,U_MINUS,te,nullptr,$$->array);
                    quad_table->emit(q);
                }else{
...
init_declarator:
    ...
    {
        typecheck(&($3),$1);
        if($1->category == SymbolTableEntry::GLOBAL){
            global_initialized.insert($1->name);
        }
...
declarator:
    pointer direct_declarator
    {
        ...
        // t->next = $2->type;
        if($2->type->type == FUNCTION || $2->type->type == ARRAY){
            t->next = $2->type->next;
            $2->type->next = t;
            $2->size = compute_type_size($2->type);
        }else{
            t->next = $2->type;
...
direct_declarator:
    ...
    |PRA RDBLT changetable_dec parameter_list RDBRT
    {
        ...
        $1.loc->category = SymbolTableEntry::FUNCT;
        // $1.loc->update(new typeNode(cur_type), $1.loc->initial_value, $1.loc->nested_table);
...
parameter_declaration:
    type_specifier pointer IDENTIFIER
    {
        ...
        $3.loc->category = SymbolTableEntry::PARAMETER;
        typeNode* t = new typeNode(PTR);
        t->next = $3.loc->type;
    ...
    |type_specifier IDENTIFIER
    {
        ...
        $2.loc->category = SymbolTableEntry::PARAMETER;
...
function_definition:
    ...
    {
        cur_symbol_table = global_symbol_table;
        quad_table->emit(new quadTableEntry($2->name,"","",FUCNEND,$2,0,0));
    }
    ;
